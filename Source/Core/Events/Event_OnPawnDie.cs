...
namespace Diary.Core.Events
{
    ...
    {
        private static RandomString _huntingSentences = new RandomString(
            ...
            "!HUNTER.NAME! tried to hunt !VICTIM.NAME!. !HUNTER.PRONOUN! fights like !NAME.ANIMAL/INDEF! but !VICTIM.NAME! got the victory and !HUNTER.NAME!'s life."
        );

        public Event_OnPawnDie(Pawn p)
        {
            if (p.CurJob.def == JobDefOf.Hunt)
            {
                CreatePawnDieDuringHuntingSentence(p);
            }
        }
